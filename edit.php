...
if (isset($_POST["submit"])) {
  ...
  $sql = "UPDATE `ETUDIANT` SET `PRENOM`='$prenom',`NOM`='$nom',`DATENAISSANCE`='$date',`FILLIERE`='$filliere',`CLASSE`='$classe',`EMAIL`='$email',`TELEPHONE`='$telephone' WHERE MATRICULE =$matricule ";

  $result = mysqli_query($conn, $sql);

  if ($result) {
    header("Location: index.php?msg=Donnée Mise à jour avec succés");
  } else {
...
<head>
  ...
  <title>CRUD Gestion Etudiant</title>
</head>

<body>
  <nav class="navbar navbar-light justify-content-center fs-3 mb-5" style="background-color: #00ff5573;">
    PHP Complete CRUD Gestion Etudiant
  </nav>
  ...
  <div class="container">
    ...
    <?php
    $sql = "SELECT * FROM `ETUDIANT` WHERE MATRICULE = $matricule LIMIT 1";
    $result = mysqli_query($conn, $sql);
